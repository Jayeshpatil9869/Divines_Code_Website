...
const CaseStudies: React.FC = () => {
  return (
    <div className="w-full bg-background-light">
      ...
      <section className="w-full overflow-hidden mb-24 py-10">
        <div className="flex w-max animate-scroll gap-8 px-4">
          {[...caseStudies, ...caseStudies].map((study, i) => (
            <div key={i} className="relative w-[90vw] max-w-[1000px] flex-shrink-0 rounded-2xl overflow-hidden shadow-2xl bg-white border border-neutral-200 transition-transform hover:scale-[1.01]">
              <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[500px] lg:h-[600px]">
                ...
                <div className="lg:col-span-4 p-8 lg:p-12 flex flex-col justify-center gap-6 z-10 bg-white lg:border-r border-neutral-100">
                  ...
                  <h2 className="text-3xl font-bold text-primary">{study.title}</h2>
                  <p className="text-neutral-500">{study.desc}</p>
                  <div className="flex flex-col gap-4 mt-4">
                    {study.stats.map((stat, j) => (
                      <div key={j} className="flex items-center gap-4">
                        <div className="bg-neutral-100 p-3 rounded-lg">
                          {j === 0 ? <Users size={20} className="text-primary" /> : <Heart size={20} className="text-primary" />}
                        </div>
                        <div>
